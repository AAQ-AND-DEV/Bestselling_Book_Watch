...
@Entity(tableName = "Bestsellers", indices = arrayOf(Index(value = ["isbn10"])))
@Parcelize
data class Bestseller(
    @PrimaryKey(autoGenerate = true) var id: Int? = null, var rank: Int? = 0, var wksPresent: Int? = 0, var isbn10: String? = "",
    var isbn13: String? ="", var publisher: String? = "", var desc: String? = "",
    var title: String? ="", var author: String? ="", var bk_img: String? ="",
    var amznUrl: String? ="", var reviewUrl: String? ="", @Embedded var buyLinks: NetworkBuyLinks? = NetworkBuyLinks(listOf(NetworkBuyLink("",""))), var categories: MutableSet<String>? = null
) : Parcelable{
//    @Ignore
